...
if(!document.querySelector('link[href="/deepdell-v2.css"]')){}
...
document.querySelectorAll('.footer-brand > a').forEach(brand=>{brand.innerHTML='<img class="footer-attached-logo" src="/assets/deepdell-footer-logo.svg" alt="DEEPDELL">';});
...
menu?.addEventListener('click',()=>{const open=header?.classList.toggle('menu-open');menu.setAttribute('aria-expanded',String(!!open))});document.addEventListener('keydown',e=>{if(e.key==='Escape'){header?.classList.remove('menu-open');menu?.setAttribute('aria-expanded','false')}});document.querySelectorAll('.nav a').forEach(a=>a.addEventListener('click',()=>{header?.classList.remove('menu-open');menu?.setAttribute('aria-expanded','false')}));
const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches,coarse=matchMedia('(pointer: coarse)').matches;let ticking=false;
const motionNodes=[...document.querySelectorAll('.hero-art,.shopify-visual,.dashboard,.architecture,.contact-panel,.page-hero>div')];const scroll=()=>{const y=scrollY,max=document.documentElement.scrollHeight-innerHeight;if(progress)progress.style.width=`${max>0?Math.min(100,y/max*100):0}%`;header?.classList.toggle('scrolled',y>20);if(!reduce){motionNodes.forEach(el=>{const r=el.getBoundingClientRect(),d=(r.top+r.height/2-innerHeight/2)/innerHeight;el.style.setProperty('--scroll-tilt',`${Math.max(-5,Math.min(5,-d*7))}deg`);el.style.setProperty('--scroll-depth',`${Math.max(-32,Math.min(32,-d*24))}px`)});const a=document.querySelector('.hero-art');if(a)a.style.setProperty('--scroll-y',`${Math.max(-38,Math.min(18,y*.035))}px`)}ticking=false};addEventListener('scroll',()=>{if(!ticking){ticking=true;requestAnimationFrame(scroll)}},{passive:true});scroll();
...
const companion=document.querySelector('.dd-companion');if(companion){const openCompanion=()=>{const target=document.querySelector('.agentic-home')||document.querySelector('a[href="agentic-check.html"]');if(target){if(target.matches('a')) location.href=target.href; else target.scrollIntoView({behavior:'smooth',block:'center'})}};companion.addEventListener('click',openCompanion);if(!reduce&&!coarse){document.addEventListener('pointermove',e=>{const x=(e.clientX/innerWidth-.5),y=(e.clientY/innerHeight-.5);companion.style.transform=`translate(${x*8}px,${y*6}px)`},{passive:true})}}
...
(function(){
  ...
  function loadGA(){
    ...
    s.onload=()=>{
      gtag('js',new Date());
      gtag('config',GA_ID,{send_page_view:true,debug_mode:false});
      gtag('consent','update',{
      ...
      gtag('event','deepdell_page_view',{page_path:location.pathname,page_title:document.title});
    };
