...
// GA4 conversion events — emitted only after consent has loaded GA.\nfunction deepdellEvent(name,params){try{if(typeof window.gtag==='function')window.gtag('event',name,params||{})}catch(e){}}\ndocument.addEventListener('click',e=>{const a=e.target.closest('a');if(!a)return;const href=a.getAttribute('href')||'';if(href.includes('agentic-check.html'))deepdellEvent('agentic_check_started',{link_location:path});if(href.includes('contact.html'))deepdellEvent('contact_cta_clicked',{link_location:path});if(href.includes('shopify-plus.html'))deepdellEvent('shopify_plus_cta_click',{link_location:path});if(href.includes('services.html'))deepdellEvent('service_cta_click',{link_location:path});if(href.includes('contact-card.html'))deepdellEvent('contact_card_clicked',{link_location:path})});\ndocument.addEventListener('submit',e=>{const id=e.target?.id||'';if(id==='home-agentic-form')deepdellEvent('agentic_check_started',{link_location:'homepage'});if(id==='scan-form')deepdellEvent('agentic_check_started',{link_location:'agentic-check'});if(id==='contact-form')deepdellEvent('contact_form_submitted',{link_location:path})});\nconst page=path.split('/').pop()||'';document.querySelectorAll('.nav a').forEach(a=>{if(a.getAttribute('href')===page)a.setAttribute('aria-current','page')});
})();
