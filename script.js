...
const companion=document.querySelector('.dd-companion');if(companion&&!reduce&&!coarse){document.addEventListener('pointermove',e=>{const x=(e.clientX/innerWidth-.5),y=(e.clientY/innerHeight-.5);companion.style.transform=`translate(${x*8}px,${y*6}px)`},{passive:true});companion.addEventListener('click',()=>{const target=document.querySelector('.agentic-home')||document.querySelector('a[href="agentic-check.html"]');if(target){if(target.matches('a')) location.href=target.href; else target.scrollIntoView({behavior:'smooth',block:'center')}}})}
